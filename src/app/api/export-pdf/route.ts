...
export async function POST(request: NextRequest) {
  try {
    ...
    const isPro = userProfile.subscriptionTier === 'pro' || userProfile.subscriptionTier === 'lifetime';

    // Use course data directly
    const course = courseData;

    // Get user branding settings
    const branding = await getUserBranding(userProfile.id);
    ...
    // Log the export action - increment usage in user profile
    // Note: You might want to implement usage tracking for Supabase auth users
